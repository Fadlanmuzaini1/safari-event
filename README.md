...
yang bukan gilirannya, dan Pokemon liar acak "common" muncul otomatis berkala selama
giliran berjalan.

> **Catatan snapshot**: ini adalah rekonstruksi source code pada titik rilis **v0.5.0**.
> Fitur teleport masuk/keluar area Safari, HUD khusus per-grup, Safari Ball berkala, dan
> pembersihan Pokemon liar antar giliran (ditambahkan di v0.6.0/v0.7.0) TIDAK ada di sini.

**Status pada snapshot ini: fitur inti + grup manual + spawn otomatis lengkap & sudah
...
│   └── CatchTracker.kt
├── spawn/                      Force-spawn manual admin + spawn otomatis berkala
...
- **`PassiveSpawnScheduler` memakai `ForceSpawnManager` yang sama persis** dengan
  ...
  oleh timer selama `EventManager.isRunning`.
...
Lihat [CHANGELOG.md](CHANGELOG.md) untuk riwayat rilis sampai dengan v0.5.0 (snapshot ini).
