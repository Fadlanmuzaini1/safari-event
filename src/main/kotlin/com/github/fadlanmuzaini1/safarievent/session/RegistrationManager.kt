...
class RegistrationManager(private val configManager: ConfigManager) {
    ...
    /** Ringkasan read-only satu grup, dipakai untuk command /safari list. */
    ...
    /**
     * Snapshot SEMUA grup beserta anggotanya saat ini, terurut dari grup 1 dst. Dipakai
     * CommandManager untuk /safari list -- admin melihat siapa saja yang sudah daftar dan
     * masuk grup mana, termasuk status locked (giliran grup itu sudah pernah di-start atau belum).
     */
    fun listGroups(): List<GroupSummary> {
        return groups.map { group ->
            GroupSummary(
                index = group.index,
                displayName = displayName(group.index),
                locked = group.locked,
                memberNames = group.members.values.toList()
            )
        }
    }
