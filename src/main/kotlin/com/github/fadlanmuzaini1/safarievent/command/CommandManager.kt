 ...
 *
 * CATATAN: /safari register, /safari unregister, /safari list, /safari reset, dan
 * /safari forcespawn TIDAK ada di daftar command awal -- ditambahkan belakangan sebagai
 * konsekuensi wajar dari fitur pendaftaran, giliran grup manual per-grup, kebutuhan admin
 * melihat siapa saja yang sudah daftar, kebutuhan mengakhiri sesi secara eksplisit, dan
 * kebutuhan memunculkan Pokemon secara paksa di area Safari.
 */
class CommandManager(
        ...
        dispatcher.register(
            literal("safari")
                ...
                .then(
                    literal("forcespawn")
                        .requires { it.hasPermissionLevel(2) }
                        .executes { ctx -> executeForceSpawn(ctx, 1) }
                        .then(
                            argument("jumlah", IntegerArgumentType.integer(1, 50))
                                .executes { ctx ->
                                    executeForceSpawn(ctx, IntegerArgumentType.getInteger(ctx, "jumlah"))
                                }
                        )
                )
                .then(
    ...
    /**
     * Menampilkan seluruh grup beserta anggotanya kepada admin -- termasuk status locked
     * (giliran grup itu sudah pernah di-start atau belum), supaya admin tahu grup mana yang
     * masih bisa di-start dan grup mana yang sudah selesai gilirannya.
     */
    private fun executeList(ctx: CommandContext<ServerCommandSource>): Int {
        val groups = registrationManager.listGroups()
        if (groups.isEmpty()) {
            ctx.source.sendFeedback({ Text.literal("Belum ada pemain yang mendaftar.") }, false)
            return 1
        }

        val message = buildString {
            append("===== Daftar Pendaftar Safari Event =====")
            for (group in groups) {
                val status = if (group.locked) "sudah di-start" else "belum di-start"
                append("\n${group.displayName} ($status, ${group.memberNames.size} pemain): ")
                append(group.memberNames.joinToString(", "))
            }
        }
    ...
    /**
     * Spawn paksa `jumlah` Pokemon acak "common" di titik acak dalam area Safari. Hasil
     * (sukses/gagal per-Pokemon) HANYA dikirim sebagai feedback ke admin yang menjalankan
     * command -- TIDAK di-broadcast ke seluruh server, karena ini murni utilitas admin
     * (setup/testing/menambah stok Pokemon), bukan momen lifecycle event seperti start/stop.
     */
    private fun executeForceSpawn(ctx: CommandContext<ServerCommandSource>, jumlah: Int): Int {
        val server = ctx.source.server
        val spawnedNames = mutableListOf<String>()
        var failureCount = 0
        var noRegionConfigured = false

        repeat(jumlah) {
            when (val result = forceSpawnManager.forceSpawnOne(server)) {
                is ForceSpawnManager.ForceSpawnResult.Success -> spawnedNames.add(result.speciesName)
                ForceSpawnManager.ForceSpawnResult.NoRegionConfigured -> noRegionConfigured = true
                ForceSpawnManager.ForceSpawnResult.WorldNotLoaded,
                ForceSpawnManager.ForceSpawnResult.SpawnFailed -> failureCount++
            }
        }

        if (noRegionConfigured) {
            ctx.source.sendError(Text.literal("Belum ada region Safari yang valid di config. Cek config/safari-event.json lalu /safari reload."))
            return 0
        }

        if (spawnedNames.isEmpty()) {
            ctx.source.sendError(Text.literal("Gagal men-spawn Pokemon sama sekali ($failureCount kegagalan). Cek log server untuk detail."))
            return 0
        }

        val message = buildString {
            append("Berhasil men-spawn ${spawnedNames.size} Pokemon di area Safari: ")
            append(spawnedNames.joinToString(", "))
            if (failureCount > 0) append(" ($failureCount gagal, cek log server)")
        }
        ctx.source.sendFeedback({ Text.literal(message) }, true)
        return spawnedNames.size
    }
