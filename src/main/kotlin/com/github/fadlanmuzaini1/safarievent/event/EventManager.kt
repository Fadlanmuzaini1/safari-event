 ...
 * SafariTeleporter, SafariBallSupplier, WildPokemonCleaner, LeaderboardManager, dan
 * EventScoringStrategy menjadi satu alur.
 *
...
class EventManager(
    ...
    private val safariTeleporter: SafariTeleporter,
    private val safariBallSupplier: SafariBallSupplier,
    private val wildPokemonCleaner: WildPokemonCleaner,
    ...
    fun tick(server: MinecraftServer) {
        ...
        safariBallSupplier.tick(server)
    }
    ...
    fun startGroup(
        ...
        safariTeleporter.teleportGroupIn(server, group)
        safariBallSupplier.grantImmediate(server, group)
    ...
    private fun handleGroupTurnFinished(finishedGroup: Group) {
        ...
        // Kembalikan pemain ke lokasi semula SEBELUM broadcast hasil, supaya mereka sudah
        // berada di luar area Safari saat menerima pesan leaderboard sementara/hasil akhir.
        safariTeleporter.teleportGroupOut(server, finishedGroup)
        // Bersihkan semua Pokemon liar yang tersisa di area Safari -- sesuai permintaan
        // eksplisit "setiap grup selesai, semua Pokemon di dalam Safari dihapus".
        wildPokemonCleaner.removeAllWildPokemon(server)
